...
Public Sub ApplyDiff(mstTbl As String, wkTbl As String, keyCD As String, _
    ...
    'ワークテーブルの金額を表示用（絶対値）から集計用（収入以外マイナス）に更新
    If fromView Then
    ...
    '挿入時にカラム順のズレによる誤挿入を防ぐため、明示的にフィールド一覧を指定
    'キーフィールドの存在を確認
...
On Error GoTo ErrHandler
    ...
    'ワークテーブルの金額を表示用（絶対値）に戻す
    db.Execute "qryViewMode", dbFailOnError
